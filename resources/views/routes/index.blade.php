        ...
        <div class="overflow-x-auto">
            <table class="min-w-full bg-white border border-gray-100 rounded-xl shadow-sm">
                <thead class="bg-gray-50">
                    <tr>
                        <th class="w-16 text-left px-6 py-4 text-sm font-medium text-gray-600">ID</th>
                ...
                <tbody>
                    @forelse ($routes as $route)
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            <td class="w-16 px-6 py-4 text-sm text-gray-700 border-b border-gray-100">{{ $route['id'] }}</td>
                            <td class="px-6 py-4 text-sm text-gray-700 border-b border-gray-100">
                                {{ $route['start_city_name'] }}</td>
                            <td class="px-6 py-4 text-sm text-gray-700 border-b border-gray-100">
                                {{ $route['end_city_name'] }}</td>
                            <td class="px-6 py-4 text-sm text-gray-700 border-b border-gray-100">
                                {{ number_format($route['base_price'], 2) }}</td>
                            <td class="px-6 py-4 text-sm text-gray-700 border-b border-gray-100">
                                {{ number_format($route['distance_km'], 2) }}</td>
                            <td class="px-6 py-4 text-sm text-gray-700 border-b border-gray-100">
                                {{ number_format($route['estimated_duration_hours'], 2) }}</td>
                            ...
                            <td class="px-6 py-4 text-sm border-b border-gray-100 space-x-3">
                                <a href="{{ route('routes.show', $route['id']) }}"
                                    class="bg-blue-500 text-white px-4 py-1.5 rounded-lg hover:bg-blue-600 transition duration-200 ease-in-out transform hover:-translate-y-0.5">Lihat</a>
                                <a href="{{ route('routes.edit', $route['id']) }}"
                                    class="bg-yellow-500 text-white px-4 py-1.5 rounded-lg hover:bg-yellow-600 transition duration-200 ease-in-out transform hover:-translate-y-0.5">Edit</a>
                                <form action="{{ route('routes.destroy', $route['id']) }}" method="POST" class="inline"
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                        class="bg-red-500 text-white px-4 py-1.5 rounded-lg hover:bg-red-600 transition duration-200 ease-in-out transform hover:-translate-y-0.5">Hapus</button>
                                </form>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-4 text-center text-sm text-gray-500 border-b border-gray-100">
                                Tidak ada data rute</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
