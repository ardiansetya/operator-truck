...
@section('content')
    <div class="container mx-auto px-6 py-8">
        ...
        <div id="transitModal"
            class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden z-[1050] flex items-center justify-center p-4">
    ...
    <script>
        ...
        function updateTruckList() {
            ...
            trucks.forEach(truck => {
                ...
                truckItem.className =
                    `truck-item p-4 bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow cursor-pointer ${selectedTruckId === truck.id ? 'ring-2 ring-blue-500 bg-blue-50' : ''}`;
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                     ...
                     <div class="flex items-start gap-2">
                        <span class="text-gray-500 shrink-0">Posisi Sekarang:</span>
                        <span class="font-medium text-gray-700 ">
                              ${truck.formatted_address}
                        </span>
                    </div>
                     <div class="flex items-start gap-2">
                        <span class="text-gray-500 shrink-0">Kota Sekarang:</span>
                        <span class="font-medium text-gray-700 ">
                              ${truck.city}
                        </span>
                    </div>
            ...
            Object.values(truckData).forEach(truck => {
                ...
                <div class="truck-popup p-1 max-w-xs">
                    ...
                    <div class="space-y-1 text-sm">
                        ...
                        <p><strong>Posisi:</strong> ${truck.formatted_address}</p>
                        <p><strong>Kota:</strong> ${truck.city}</p>
                        ${routeInfoForPopup}
                    </div>
                </div>
            `;
        ...
        `;
            ...
            truck.transits.forEach((transit, index) => {
                ...
                const statusBadge = transit.is_accepted ?
                    '<span class="px-2 py-1 bg-green-100 text-green-800 text-xs rounded-full">✓ Diterima</span>' :
                    '<span class="px-2 py-1 bg-yellow-100 text-yellow-800 text-xs rounded-full">⏳ Pending</span>';
                const arrivedAt = transit.arrived_at ?
                    `<div class="text-sm text-gray-600 mt-2"><strong>Tiba:</strong> ${formatDateTime(transit.arrived_at * 1000)}</div>` :
                    '';
                const actionedAt = transit.actioned_at ?
                    `<div class="text-sm text-gray-600"><strong>Diproses:</strong> ${formatDateTime(transit.actioned_at * 1000)}</div>` :
                    '';
        ...
        function closeTransitModal() {
            document.getElementById('transitModal').classList.add('hidden');
        }
