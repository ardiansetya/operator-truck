...
<div class="container mx-auto px-6 py-8">
    <h1 class="text-2xl font-semibold text-gray-800 mb-6">Edit Kota</h1>
    ...
    <form method="POST" action="{{ route('cities.update', $city['id']) }}" class="max-w-lg bg-white p-6 rounded-xl shadow-sm">
        @csrf
        @method('PUT')
        <div class="mb-4">
            <label for="name" class="block text-sm font-medium text-gray-600">Nama Kota</label>
            <input type="text" name="name" id="name" value="{{ old('name', $city['name']) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
            @error('name')
